...
export function RefundButton({ orderId, amount }: { orderId: string; amount: string }) {
  ...
  return (
    <form action={action} className="flex items-center gap-2">
      ...
      <button type="submit" disabled={pending} className="inline-flex min-h-10 items-center rounded-(--radius-sm) bg-coral px-3 text-[0.875rem] font-semibold text-on-orange disabled:opacity-60">
