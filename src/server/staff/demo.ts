...
export function buildStaffDemo(now = new Date(), options: { userIds?: boolean } = {}): Record<string, Row[]> {
  ...
  const training_modules: Row[] = [
    lesson('welcome', { slug: 'welcome-to-casa-aurelia', title: 'Welcome to Casa Aurelia', description: 'Who we are, how a night runs, and what guests remember.', estimated_minutes: 10 }),
    lesson('scanner', { slug: 'door-scanner', title: 'Door & QR scanner', description: 'Checking tickets in with the Casa Aurelia scanner, what each result means, and when to get a manager.', category: 'door', estimated_minutes: 15, passing_score: 80, applies_to_positions: ['door', 'security', 'manager'], retrain_interval_days: 365 }),
    lesson('alcohol', { slug: 'alcohol-service', title: 'Alcohol service', description: 'Checking IDs, cutting someone off kindly, and what Casa Aurelia policy says.', category: 'bar', estimated_minutes: 20, passing_score: 80, applies_to_positions: ['bartender', 'server', 'manager'], retrain_interval_days: 365, version: 2 }),
  ...
  const training_sections: Row[] = [
    section('welcome', 10, { title: 'What Casa Aurelia is', body: 'Casa Aurelia is an Italian supper club in the West Loop that turns into a venue at night: Vinyl & Vermouth, After Hours Saturday, birthdays, private events. Guests come for the food and stay for the room. Your job, whatever your position, is that they leave planning to come back.' }),
    section('welcome', 20, { title: 'A night at Casa Aurelia', body: 'Doors open at the time on the event. The host greets, the door scans tickets, the bar and floor keep pace with the room. The manager on duty makes the calls; when you are not sure, ask them before a guest notices.' }),
    ...
    section('scanner', 10, { title: 'The scanner', body: 'The door phone opens the Casa Aurelia scanner at /admin/scan. Point the camera at the QR on the guest’s phone or printout. Green means in; hold the phone steady and read the name back.' }),
    ...
    section('alcohol', 30, { kind: 'link', title: 'Casa Aurelia alcohol policy', body: 'The full policy is in your documents. Read it once a year.', media_url: '/staff/documents' }),
  ...
  const quiz = [
    ...
    question('alcohol', 10, 'multiple_choice', 'Which of these is acceptable ID at Casa Aurelia?', ['An expired driver’s licence', 'A photo of a passport on a phone', 'A military ID', 'A college student card'], [2], 'A real, unexpired government ID. Photos of IDs are not IDs.'),
